...
##############################################################################
##read into gene expression (FPKM) among 79 tissues and cell types in sheep, 
...
for(i in 1:length(Samples_sets)){
  ...
  if(No_tissues_category==1){
    ...
    write.table(Myres,file = paste("Output_path_",Tissue_class,"_t.txt",sep = ""),append = F,quote = F,row.names = T,col.names = F)
  }
  ...
  if(Tissue_class=="Other"){
    for(k in 1:length(tissues_unique[["Other"]])){
      ...
      row.names(Myres) <- row.names(X)
      write.table(Myres,file = paste("Output_path_",Other_tissues[k],"_t.txt",sep = ""),append = F,quote = F,row.names = T,col.names = F)
    }
  }
  ...
  if(Tissue_class!="Other"&No_tissues_category>1){
    ...
    for(j in 1:length(Tissue_analyzed)){
      ...
      head(Myres)
      row.names(Myres) <- row.names(X)
      write.table(Myres,file = paste("Output_path_",target_tissue,"_t.txt",sep = ""),append = F,quote = F,row.names = T,col.names = F)
    }
  }
}
